    ...
    <>
      ...
      <div className="kpi-row">
        <div className="kpi-card">
          ...
          <div className={`kpi-change ${varLabor <= 0 ? 'pos' : 'neg'}`}>Bud: {fmtPct(total.laborBud)} · Var: {fmtVarPC(-varLabor)}</div>
        </div>
        <div className="kpi-card">
          <div className="kpi-label">Avg COGS %</div>
          <div className="kpi-value">{fmtPct(total.cogsAct)}</div>
          <div className={`kpi-change ${varCogs <= 0 ? 'pos' : 'neg'}`}>Bud: {fmtPct(total.cogsBud)} · Var: {fmtVarPC(-varCogs)}</div>
        </div>
        <div className="kpi-card">
          <div className="kpi-label">Prime Cost %</div>
          <div className="kpi-value">{fmtPct(total.pcAct)}</div>
          <div className={`kpi-change ${varPC <= 0 ? 'pos' : 'neg'}`}>Bud: {fmtPct(total.pcBud)} · Var: {fmtVarPC(-varPC)}</div>
        </div>
      </div>
