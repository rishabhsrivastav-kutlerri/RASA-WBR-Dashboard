...
function parseWorksheetCF(sheetXml, stylesXml) {
  ...
  for (const block of blocks) {
    ...
    for (const rm of ruleMatches) {
      ...
      // Formula text is inconsistent even within one rule set — sometimes a
      // quoted percent literal (XML-encoded as &quot;8%&quot;), sometimes a
      // bare number (6, 8) that's actually meant in the same percent-point
      // scale. Stripping quotes/% here normalizes both to that same
      // percent-point scale; the caller compares pct-formatted metrics
      // against value*100 to match (see cellBg in xlsxParser.js).
      const formulas = (rm.match(/<formula>([^<]*)<\/formula>/g) || [])
        .map(f => {
          let s = /<formula>([^<]*)<\/formula>/.exec(f)[1].replace(/&quot;/g, '"').trim();
          const quoted = /^"(.*)"$/.exec(s);
