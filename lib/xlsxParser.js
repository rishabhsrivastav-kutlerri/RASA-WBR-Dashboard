  ...
  if (wbrFile) {
    ...
    if (pcrFile) {
      ...
      if (pcrRows.length > 4) {
        ...
        // Same PCR sheet, no budget available for these columns (left empty).
        ...
        if (weekInfo && weekInfo.period === 7 && weekInfo.weekInPeriod === 2) {
          ...
          const buildTrailingCostRows = pcrByLoc => TRAILING_LOC_ORDER
            .filter(loc => pcrByLoc[loc])
            .map(loc => {
              const p = pcrByLoc[loc];
              return {
                loc,
                laborAct: p.laborAct, laborBud: null,
                cogsAct:  p.cogsAct,  cogsBud:  null,
                pcAct:    p.laborAct + p.cogsAct, pcBud: null,
                varPC: null,
                primeMarginAct: p.primeMarginAct,
              };
            });
          trailing4 = { costs: buildTrailingCostRows(pcr.trailing4) };
          trailing8 = { costs: buildTrailingCostRows(pcr.trailing8) };
