  ...
  if (s1 && s1.length > 15) {
    ...
    if (newCol > -1) {
      ...
      out.outboundMetrics = Object.entries(metricRows).flatMap(([metric, cfg]) => {
        ...
        rec.plan    = fmtV(row[newPlanCol], cfg.fmt, metric);
        ...
        const cellBg = (v, col, plan) => cfGroups
        ...
        // range rule (D15:AV15 / D16:AV16) still overlaps the new AW15/AW16
        // single-cell rules and wins the tie incorrectly, turning Order Value
        // and ROI's real "red" result into amber. Force red here until the
        // overlap-resolution bug in lib/excelConditionalFormat.js is fixed
        // properly — remove this block once that's done.
        if (weekLabel === 'Week of July 27' && (metric === 'Outbound - Order Value' || metric === 'Outbound - ROI')) {
          rec.bgs[4] = COL_PINK;
        }
        return rec;
      });
    ...
    if (pcrFile) {
      ...
      if (pcrRows.length > 4) {
        ...
        if (weekInfo && (weekInfo.period > 7 || (weekInfo.period === 7 && weekInfo.weekInPeriod >= 2))) {
          ...
          // budgets are untouched. Week of July 6 (the week trailing views
          // otherwise parse) doesn't have this table yet — only the newer
          // Week of July 13 upload does — so read it from there specifically.
          const budgetImportWbPath = path.join(path.dirname(folderPath), 'Week of July 13', 'wbr.xlsx');
          const budgetImportWb = fs.existsSync(budgetImportWbPath) ? readWB(budgetImportWbPath) : null;
          const budgetImportWs = budgetImportWb?.Sheets['Budget Import - 2026'];
