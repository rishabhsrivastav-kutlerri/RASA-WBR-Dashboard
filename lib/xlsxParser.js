...
// Parse "Budget Import - 2026" into { weekNum: { weekly:{}, ptd:{} } }.
// weekNum is the sequential fiscal week (1-52); values are pre-calculated budgets.
function parseBudgetImport(wb) {
  const rows = sheetRows(wb, 'Budget Import - 2026');
...
function parseWBR(wb) {
  ...
  // Pre-calculated weekly/PTD budgets per RC, keyed by fiscal week number.
  const budgetByWeek = parseBudgetImport(wb);
...
export function parseWeekFolder(folderPath, weekLabel) {
  ...
  if (wbrFile) {
    ...
    // Inject weekly and PTD per-RC budget from Budget Import - 2026.
    // The sheet pre-calculates exact weekly and PTD values by fiscal week number.
    if (W.rcBudgetByWeek) {
