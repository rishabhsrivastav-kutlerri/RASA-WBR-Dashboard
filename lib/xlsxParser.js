    ...
    function readApolloEmailTable(anchor) {
      ...
      for (let i = anchor.r + 2; i < catRows.length; i++) {
        ...
        out.push({
          ...
          delivered: num(r[anchor.c + 2]),
          bounced:   num(r[anchor.c + 3]),
          opened:    num(r[anchor.c + 4]),
          replied:   num(r[anchor.c + 5]),
        });
      }
      return out;
    }
...
function parseLoyalty(wb) {
  ...
  if (lc.length > 2) {
    ...
    function findLifecycleRow(label) {
      for (let i = 10; i < lc.length; i++) {
        const r = lc[i]; if (!r) continue;
  ...
  // MoM trend — monthly Loyalty Signups + App Downloads from MoM 2026 sheet.
  // Row 2 (index 2) has month-end dates; row 24 has signups; row 25 has app downloads.
  // Only include months where both signups > 0 (non-zero = data was reported).
  const mom2026 = ws('MoM 2026');
  if (mom2026.length > 25) {
    const dateRow    = mom2026[2]  || [];
    const signupsRow = mom2026[24] || [];
    const appDlRow   = mom2026[25] || [];
    const months = [], momSignups = [], momAppDl = [];
    for (let c = 1; c < dateRow.length; c++) {
      const d = dateRow[c];
      const sv = num(signupsRow[c]);
      if (!(d instanceof Date) || sv === 0) continue;
      months.push(d.toLocaleString('en-US', { month: 'short', year: '2-digit' }));
      momSignups.push(sv);
      momAppDl.push(num(appDlRow[c]));
    }
    if (months.length) {
      W.lifecycleMomTrend = { months, signups: momSignups, appDownloads: momAppDl };
    }
  }
