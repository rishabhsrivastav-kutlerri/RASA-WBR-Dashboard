...
async function parseCateringWB(wb, weekLabel, buffer) {
  ...
  // "Location Wise" tab — per-location breakdown of the same Outbound team
  // metrics, added starting Week of Sep 7. The sheet lays out one block of
  // columns per week (Metrics, then one column per location, then Total),
  // and a new block is appended further right each week — so rather than
  // hardcode a column, find THIS week's block by searching row 1 for the
  // cell that reads exactly "Week of <monthDay>" (older stale blocks are
  // headed "Week of <X> Total" or similar, which this exact match skips).
  // Weeks before Sep 7 have no such cell yet, so this naturally produces
  // nothing for them — no per-week gating needed.
  const lw = ws('Location Wise');
  if (lw && lw.length > 2) {
    const lwLabelRow = lw[0] || [];
    let lwCol = -1;
    for (let c = 0; c < lwLabelRow.length; c++) {
      const h = lwLabelRow[c];
      if (h != null && safeStr(h).replace(/\n/g, ' ').trim() === 'Week of ' + monthDay) { lwCol = c; break; }
    }
    if (lwCol > -1) {
      // This block's own header row (one row below the week label) carries
      // "Metrics" in lwCol, then one location per column up to and including
      // "Total" — read the location list from there instead of hardcoding
      // it, so an added/removed location doesn't need a code change.
      const lwHdrRow = lw[1] || [];
      const lwLocCols = [];
      for (let c = lwCol + 1; c < lwHdrRow.length; c++) {
        const h = safeStr(lwHdrRow[c] || '').trim();
        if (!h) break;
        lwLocCols.push({ c, label: h });
        if (/^total$/i.test(h)) break;
      }
      const LW_PCT_METRICS = new Set(['% Calls Connected', '% of calls ≥ 90 sec', '% of calls >= 90 sec']);
      const fmtLwCell = (v, metric) => {
        if (typeof v !== 'number') return '-';
        if (LW_PCT_METRICS.has(metric)) return Math.round(v * 100) + '%';
        if (metric === 'Outbound - Order Value') return '$' + Math.round(v).toLocaleString();
        if (metric === 'Outbound - ROI') return (Math.round(v * 100) / 100).toFixed(2);
        return String(Math.round(v));
      };
      // A Map (not an array) so that if the sheet repeats a metric label
      // within one week's block (seen for "# Emails / Week"/"# Opens" in
      // Week of Sep 14, where a stale all-zero row precedes the real one),
      // the later occurrence's values win while the row keeps its FIRST
      // position — Map.set on an existing key updates the value in place
      // without moving it. A well-formed block with no repeats (every other
      // week) behaves exactly as a plain push would.
      const lwMap = new Map();
      for (let r = 2; r < lw.length; r++) {
        const row = lw[r];
        const metric = row ? safeStr(row[lwCol] || '').trim() : '';
        if (!metric) break;
        const rec = { metric };
        for (const lc of lwLocCols) rec[lc.label] = fmtLwCell(row[lc.c], metric);
        lwMap.set(metric, rec);
      }
      const lwRows = Array.from(lwMap.values());
      if (lwRows.length && lwLocCols.length) {
        out.locationWiseMetrics = { cols: lwLocCols.map(lc => lc.label), rows: lwRows };
      }
    }
  }
