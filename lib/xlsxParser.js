...
async function parseCateringWB(wb, weekLabel, buffer) {
  ...
  if (lw && lw.length > 2) {
    ...
    if (lwCol > -1) {
      ...
      // "# Emails / Week" and "# Opens" don't belong in this table — dropped
      // unconditionally regardless of week.
      const LW_EXCLUDED_METRICS = new Set(['# Emails / Week', '# Opens']);
      const lwRows = [];
      for (let r = 2; r < lw.length; r++) {
        const row = lw[r];
        const metric = row ? safeStr(row[lwCol] || '').trim() : '';
        if (!metric) break;
        if (LW_EXCLUDED_METRICS.has(metric)) continue;
        const rec = { metric };
