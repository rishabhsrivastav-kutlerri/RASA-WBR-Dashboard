...
async function githubDirectUpload(weekName, fileType, file) {
  ...
  // PCR workbook lives under a separate root (PCR/<week>/) from wbr/loyalty/catering (data/<week>/).
  const root = fileType === 'pcr' ? 'PCR' : 'data';
  const newFilePath = `${root}/${weekName}/${fileType === 'pcr' ? 'PCR' : fileType}.xlsx`;

  // 2. Find any existing file for this type in the week folder (may have a non-standard name).
  let oldFilePath = null;
  const dirRes = await gh(`/repos/${owner}/${repo}/contents/${root}/${encodeURIComponent(weekName)}?ref=${branch}`);
  if (dirRes.ok) {
...
export async function fetchScorecard(granularity, item) {
  const cacheKey = `wbr_sc_${granularity}_${item}`;
  const cached = scGet(cacheKey);
  if (cached) return cached;
