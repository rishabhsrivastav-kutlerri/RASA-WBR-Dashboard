...
function weeklyItem(filename) {
  ...
  if (m) {
    const [, mon1, d1, y1, mon2, d2] = m;
    const fiscal = weekInfoForLabel(`${mon1} ${d1}`);
    // Always spell out both the start and end month, 3-letter abbreviated,
    // even when they're the same month (e.g. "Jun 8– Jun 14") — consistent
    // and unambiguous regardless of whether the week crosses a month boundary.
    const abbr3 = s => s.charAt(0).toUpperCase() + s.slice(1, 3).toLowerCase();
    const range = `${abbr3(mon1)} ${d1}– ${abbr3(mon2)} ${d2}`;
...
// Parse an already-loaded workbook buffer into { dashboard, matrix }.
// Shared by the build-time (local file) and runtime (GitHub download) paths.
export function loadScorecardFromBuffer(granularity, buffer) {
...
// Load one scorecard selection → its dashboard + the scoring matrix, from the
// local scorecard/<granularity>/ folder. Build-time use only (precompute.mjs);
// the runtime fallback in app/api/scorecard/route.js downloads from GitHub
// instead, since the local folder isn't shipped to the deployed function.
export function loadScorecard(granularity, id) {
  ...
  return loadScorecardFromBuffer(granularity, fs.readFileSync(file));
}
